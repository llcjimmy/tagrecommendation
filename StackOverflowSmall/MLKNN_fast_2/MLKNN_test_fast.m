function [HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Pre_Labels]=MLKNN_test_fast(train_data,train_target,test_data,test_target,Num,Prior,PriorN,Cond,CondN,ProTest,KeyCondLabel,KeyCondLabelN,NKeyCondLabel,NKeyCondLabelN)
...
%         end
        ...
        for j=1:num_class
            ...
            % lilicheng add
            if(ProTest(i,j) > Prior(j))
                Prob_in=(ProTest(i,j)*KeyCondLabel(1,j)+NKeyCondLabel(1,j)*(1-ProTest(i,j)))*Cond(j,temp(1,j)+1);
                Prob_out=(ProTest(i,j)*KeyCondLabelN(1,j)+NKeyCondLabelN(1,j)*(1-ProTest(i,j)))*CondN(j,temp(1,j)+1);
            end
