function [Prior,PriorN,Cond,CondN,KeyCondLabel]=MLKNN_train_fast(train_data,train_target,Num,Smooth,ProTrain)
...
%     end
    ...
    T1=lsh('e2lsh',50,100,size(train_data,2),train_data','range',500);
...
%         Neighbors{i,1}=index(1:Num);
        if(length(nnlsh)~=Num+1)
            disp(strcat('error finding for instance:',num2str(i)));
        end
        Neighbors{i,1}=nnlsh(2:Num+1);
    end
