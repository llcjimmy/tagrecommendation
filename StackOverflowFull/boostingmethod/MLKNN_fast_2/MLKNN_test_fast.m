function [HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Pre_Labels]=MLKNN_test_fast(train_data,train_target,test_data,test_target,Num,Prior,PriorN,Cond,CondN,ProTest,KeyCondLabel,KeyCondLabelN,T1)
...
%         [temp,index]=sort(dist_matrix(i,:),'descend');
        ...
        nnlsh=lshLookupTwo(train_data,test_data(i,:),T1,Num);
...
%         end
        ...
        for j=1:num_class
            ...
            if(ProTest(i,j) > Prior(j))
                Prob_in=ProTest(i,j)*KeyCondLabel(1,j)*Cond(j,temp(1,j)+1);
                Prob_out=ProTest(i,j)*KeyCondLabelN(1,j)*CondN(j,temp(1,j)+1);
            end
